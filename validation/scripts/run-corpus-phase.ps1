...
.PARAMETER BuildTool
  ...
  are ignored in Gradle mode. Per D-23, the host build tool is otherwise
  irrelevant to coverdict itself - only this harness script cares.
...
if ($BuildTool -eq "Gradle" -and -not $GradleModule) {
    throw "-GradleModule is required when -BuildTool Gradle"
}
...
try {
    ...
    $jacocoXml = if ($BuildTool -eq "Maven") {
        Join-Path $ModuleRoot "target\site\jacoco\jacoco.xml"
    } else {
        Join-Path $ModuleRoot "build\reports\jacoco\test\jacocoTestReport.xml"
    }
