...
| `--diagnostics-dir` | Directory for per-module L2/L3 subprocess logs. Off by default | off |
| `--diagnostics-level` | `errors` (default) keeps the engine quiet, so the log holds its failures and its summary; `verbose` adds a line per mutant per test — the only way to see why a coverage minion died (D-64), and what reached 184 MB for one module on a real repo (D-91). Each log is capped at 25 MB and says so if truncated (D-89) | `errors` |
